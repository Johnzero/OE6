...
class hr_holidays_status(osv.osv):
    ...
    def button_dummy(self, cr, uid, ids, employee_id, return_false=None, context=None):
        print uid,ids,employee_id
        print context
...
class hr_holidays(osv.osv):
    ...
    def button_dummy(self, cr, uid, ids, employee_id, return_false=None, context=None):
        ...
        print context
        print"***"*30
        cr.execute("""SELECT employee_id FROM hr_holidays""")
        val = cr.dictfetchall()
        print val
        cr.execute("""SELECT id, type, number_of_days, holiday_status_id FROM hr_holidays WHERE employee_id = %s AND state='validate' AND holiday_status_id in %s""",
            [employee_id["uid"], tuple(ids)])
